...
if ( ! class_exists( 'YITH_WC_Dynamic_Pricing_Helper' ) ) {
	...
	class YITH_WC_Dynamic_Pricing_Helper {
		...
		public function get_roles() {
			...
			return array_merge( array( '' => '', 'guest' => __( 'Guest', 'ywdpd' ) ), $roles );
		}
...
//				error_log(print_r(WC()->cart->cart_contents[ $cart_item_key ], true));
		 ...
		 * @return bool
		 */
		function valid_num_of_orders( $num, $rule ) {
		 ...
		 * @return bool
		 */
		function valid_max_num_of_orders( $num, $rule ) {
		 ...
		 * @return bool
		 */
		function valid_amount_spent( $limit, $rule ) {
		 ...
		 * @return bool
		 */
		function valid_max_amount_spent( $limit, $rule ) {
		 ...
		 * @return bool
		 */
		function valid_sum_item_quantity( $quantity, $rule ) {
		 ...
		 * @return bool
		 */
		function valid_sum_item_quantity_less( $quantity, $rule ) {
			$num_items = WC()->cart->get_cart_contents_count();
		 ...
		 * @return bool
		 */
		function valid_count_cart_items_less( $quantity, $rule ) {
		 ...
		 * @return bool
		 */
		function valid_count_cart_items_at_least( $quantity, $rule ) {
			$item_quantity = apply_filters( 'ywdpd_get_cart_item_quantities', WC()->cart->get_cart_item_quantities() );
			if ( is_array( $item_quantity ) ) {
				if ( array_sum( $item_quantity ) >= $quantity ) {
					return true;
		 ...
		 * @return bool
		 */
		function valid_subtotal_at_least( $limit, $rule ) {
		 ...
		 * @return bool
		 */
		function valid_subtotal_less( $limit, $rule ) {
			//$subtotal = apply_filters( 'ywdpd_subtotal_at_least', YITH_WC_Dynamic_Pricing()->get_option( 'calculate_discounts_tax' ) == 'tax_excluded' ? WC()->cart->subtotal_ex_tax : WC()->cart->subtotal );
			$subtotal = YITH_WC_Dynamic_Discounts()->get_cart_subtotal_to_discount( $rule );
			if ( $subtotal <= $limit ) {
				return true;
