...
if ( !class_exists( 'YITH_WC_Dynamic_Pricing_Admin' ) ) {
	...
	class YITH_WC_Dynamic_Pricing_Admin {
		...
		public function duplicate_discount() {
			...
			if ( ! current_user_can( 'manage_options' ) || ! isset( $_REQUEST['action'] ) || $_REQUEST['action'] != 'duplicate_discount' || ! isset( $_REQUEST['post'] ) ) {
				return;
			}
			global $wpdb;
			...
			if ( $results ) {
				foreach ( $results as $result ) {
					if( $result['post_type'] == 'ywdpd_discount' ){
						unset( $result['ID']  );
		...
		public function get_tags_via_ajax() {
			...
			if ( $terms = get_terms( apply_filters( 'ywdpd_json_search_tags_args', $args, $search_text ) ) ) {
		...
		public function get_categories_via_ajax() {
			...
			if ( $terms = get_terms( apply_filters( 'ywdpd_json_search_categories_args', $args, $search_text ) ) ) {
				foreach ( $terms as $term ) {
					$term->formatted_name .= $term->name . ' (' . $term->count . ')';
		...
		public function delete_transient( $post_id, $post ){
		    if( $post->post_type == 'ywdpd_discount' ){
