...
class Updater {
	...
	static function get_plugin_update_info( $plugin_slug, $latest_version ) {
		...
		$obj->icons = [
			'default' => AW()->url( '/assets/img/automatewoo-icon.png' ),
		];
		...
		if ( Licenses::is_primary( $plugin_slug ) ) {
			...
			if ( Licenses::is_valid( $plugin_slug ) && ! Licenses::is_expired( $plugin_slug ) ) {
			...
			if ( Licenses::is_valid( $addon->id ) && ! Licenses::is_expired( $addon->id ) ) {
	...
	static function in_plugin_update_message( $plugin_data ) {
		...
		elseif ( ! Licenses::is_valid( $product_id ) ) {
	 ...
	 * @return bool|object
	 */
	static function filter_plugin_api_info( $false, $action, $arg ) {
		if ( ! isset( $arg->slug ) || $arg->slug !== AW()->plugin_slug ) {
			return $false;
		}

		$response = Licenses::remote_get( 'app_wp_plugin_info' );
		...
		// WP will sanitize this data, no need to do it here
		return $info;
	}
