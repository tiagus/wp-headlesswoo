...
if ( ! class_exists( 'Astra_Ext_Adv_Search_Shortcodes' ) ) {
	...
	class Astra_Ext_Adv_Search_Shortcodes {
		...
		function enqueue_scripts() {
			...
			wp_register_style( 'advanced-search-shortcode', $css_uri . 'advanced-search-shortcode' . $file_prefix . '.css' );
		}
		...
		function search_markup( $atts ) {
			...
			if ( 'inline' === $atts['style'] ) {
				...
				add_action(
					'wp_footer',
					function() {
						echo Astra_Ext_Adv_Search_Markup::get_instance()->get_search_form( 'full-screen' );
					}
				);
