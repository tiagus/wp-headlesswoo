...
if ( ! class_exists( 'Astra_Minify' ) ) {
	...
	class Astra_Minify {
		 ...
		 */
		static private $astra_filesystem = null;
		 ...
		 * @var string $_js_key
		 */
		static private $_js_key = 'astra_theme_js_key';
		 ...
		 * @var array $js
		 */
		static private $dependent_js_files = array();
		...
		function __construct() {
			...
			add_action( 'astra_admin_settings_save', __CLASS__ . '::refresh_assets', 11 );
		...
		public static function get_dependent_js() {
			...
			if ( is_array( $dependent_js_files ) && count( $dependent_js_files ) > 0 ) {
				...
				foreach ( $dependent_js_files as $handle => $src ) {
					...
					if ( '' != $src ) {
						wp_register_script( $handle, $src, ASTRA_EXT_VER, true );
						$js_files_arr[] = $handle;
					} else {
						$js_files_arr[] = $handle;
					}
