...
/** 
 * DUP_PRO_OAuthSimple - A simpler version of OAuth
 *
...
class DUP_PRO_OAuthSimple {
    ...
    public function setURL ($path)
	{
        if (empty($path))
		{
            throw new DUP_PRO_OAuthSimpleException('No path specified for OAuthSimple.setURL');
		}
        $this->_path=$path;
    ...
    public function setAction ($action) 
    {
		...
		{
            throw new DUP_PRO_OAuthSimpleException('Invalid action specified for OAuthSimple.setAction');
		}
        $this->_action = $action;
    ...
    public function signatures ($signatures) 
    {
		...
		{
            throw new DUP_PRO_OAuthSimpleException('Must pass dictionary array to OAuthSimple.signatures');
		}
        if (!empty($signatures))
        {
		...
		{
            throw new DUP_PRO_OAuthSimpleException('Missing required consumer_key in OAuthSimple.signatures');
        }
        if (empty($this->_secrets['shared_secret']))
		{
            throw new DUP_PRO_OAuthSimpleException('Missing requires shared_secret in OAuthSimple.signatures');
		}
        if (!empty($this->_secrets['oauth_token']) && empty($this->_secrets['oauth_secret']))
		{
            throw new DUP_PRO_OAuthSimpleException('Missing oauth_secret for supplied oauth_token in OAuthSimple.signatures');
		}
    ...
    public function setSignatureMethod ($method="")
	{
        ...
        switch($method)
        {
            ...
            case 'HMAC-SHA1':
				...
				break;
            default:
                throw new DUP_PRO_OAuthSimpleException ("Unknown signing method $method specified for OAuthSimple.setSignatureMethod");
				break;
        }
    ...
    */
    ...
    private static function _oauthEscape($string) 
    {
        ...
        if (is_array($string)) {
            throw new DUP_PRO_OAuthSimpleException('Array passed to _oauthEscape');
		}
    ...
    private function _getApiKey() 
    {
        if (empty($this->_secrets['consumer_key']))
        {
            throw new DUP_PRO_OAuthSimpleException('No consumer_key set for OAuthSimple');
        }
    ...
    private function _getAccessToken() 
    {
		...
		{
            throw new DUP_PRO_OAuthSimpleException('No access token (oauth_token) set for OAuthSimple.');
		}
    ...
    private function _generateSignature () 
    {
        ...
        switch($this->_parameters['oauth_signature_method'])
        {
            ...
            default:
                throw new DUP_PRO_OAuthSimpleException('Unknown signature method for OAuthSimple');
				break;
        }
    }
}

class DUP_PRO_OAuthSimpleException extends Exception {
