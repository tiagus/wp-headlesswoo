    ...
    */
    ...
    function _connect()
    {
        ...
        if (!is_resource($this->fsock)) {
            ...
            if (!$this->fsock) {
                ...
                if($errno==111) {
                    $errstr.=". Please check if port $this->port is open on current server to make outgoing communication with $this->host.";
                }elseif($errno==110) {
                    $errstr.=". It seems that $this->host is unable to accept connection at port $this->port.";
                }
                throw new \RuntimeException(rtrim("Cannot connect to $host. $errstr"));
                return false;
            }
        ...
        if (ord($response[0]) != NET_SSH2_MSG_KEXINIT) {
            throw new \RuntimeException('Expected SSH_MSG_KEXINIT');
            return false;
        }
    ...
    function _key_exchange($kexinit_payload_server)
    {
        ...
        if ($encryptKeyLength === null) {
            throw new \RuntimeException('No compatible client to server encryption algorithms found');
            return $this->_disconnect(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
        }
        ...
        if ($kex_algorithm === false) {
            throw new \RuntimeException('No compatible key exchange algorithms found');
            return $this->_disconnect(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
        }
        ...
        if ($type != $serverKexReplyMessage) {
            throw new \RuntimeException('Expected SSH_MSG_KEXDH_REPLY');
            return false;
        }
        ...
        if ($server_host_key_algorithm === false) {
            throw new \RuntimeException('No compatible server host key algorithms found');
            return $this->_disconnect(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
        }
        ...
        if ($type != NET_SSH2_MSG_NEWKEYS) {
            throw new \RuntimeException('Expected SSH_MSG_NEWKEYS');
            return false;
        }
        ...
        if ($mac_algorithm === false) {
            throw new \RuntimeException('No compatible client to server message authentication algorithms found');
            return $this->_disconnect(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
        }
        ...
        if ($mac_algorithm === false) {
            throw new \RuntimeException('No compatible server to client message authentication algorithms found');
            return $this->_disconnect(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
        }
        ...
        if ($compression_algorithm === false) {
            throw new \RuntimeException('No compatible server to client compression algorithms found');
            return $this->_disconnect(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
        }
        ...
        if ($compression_algorithm === false) {
            throw new \RuntimeException('No compatible client to server compression algorithms found');
            return $this->_disconnect(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
        }
    ...
    function _login_helper($username, $password = null)
    {
        ...
        if (!($this->bitmap & self::MASK_LOGIN_REQ)) {
            ...
            if ($type != NET_SSH2_MSG_SERVICE_ACCEPT) {
                throw new \RuntimeException('Expected SSH_MSG_SERVICE_ACCEPT');
                return false;
            }
    ...
    function getServerPublicHostKey()
    {
        ...
        switch ($this->signature_format) {
            case 'ssh-dss':
                ...
                if (!$v->equals($r)) {
                    throw new \RuntimeException('Bad server signature');
            ...
            case 'ssh-rsa':
                ...
                */
                ...
                break;
            default:
                throw new \RuntimeException('Unsupported signature format');
                return $this->_disconnect(NET_SSH2_DISCONNECT_HOST_KEY_NOT_VERIFIABLE);
        }
