...
class SFTP extends SSH2
{
    ...
    */
     ...
     */
    function put($remote_file, $data, $mode = self::SOURCE_STRING, $time_limit = -1, $start = -1, $local_start = -1, $progressCallback = null)
    {
        if (!($this->bitmap & SSH2::MASK_LOGIN)) {
            return false;
        }
        ...
        $i = 0;
        $time_limit_start = microtime(true);
        ...
        while ($dataCallback || $sent < $size) {
            ...
            if( $time_limit > -1 ) {
                if (microtime(true) - $time_limit_start >= $time_limit) {
        ...
        if( $time_limit_reached ) {
            return array('size'=>$size,'sent'=>$sent);
        }
