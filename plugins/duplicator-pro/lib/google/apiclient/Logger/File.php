 ...
 */
class Duplicator_Pro_Google_Logger_File extends Duplicator_Pro_Google_Logger_Abstract
{
  /**
  ...
  public function __construct(Duplicator_Pro_Google_Client $client)
  {
    ...
    $file = $client->getClassConfig('Duplicator_Pro_Google_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('Duplicator_Pro_Google_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('Duplicator_Pro_Google_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new Duplicator_Pro_Google_Logger_Exception('File pointer is no longer available');
    }
