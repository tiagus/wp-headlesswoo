...
require_once(DUPLICATOR_PRO_PLUGIN_PATH . '/classes/class.php.log.php');

if( isset($_POST['clear_log']) && $_POST['clear_log'] == 'true' )
{
    DUP_PRO_PHP_Log::clear_log(DUP_PRO_PHP_Log::get_path());
}
...
$error_log = DUP_PRO_PHP_Log::get_log(NULL,$lines,"M d, H:i:s");
...
if($log_path !== false){
    ...
    if(!is_readable($log_path)){
        $error = sprintf(DUP_PRO_U::__("PHP error log is available on location %s but is not readable. Try setting the permissions to 755."),'<b>'.esc_html($log_path).'</b>');
    } else if($error_log === NULL){
        if($log_path > (PHP_INT_MAX / 2))
            $error = sprintf(DUP_PRO_U::__("PHP error log is available on location %s but can't be read because file size is over %s. You must open this file manualy."),'<b>'.esc_html($log_path).'</b>', '<b>'.DUP_PRO_U::byteSize($log_path_size).'</b>');
        else
            $error = sprintf(DUP_PRO_U::__("PHP error log is available on location %s but can't be read because some unexpected error. Try to open file manually and investigate all problems what can cause this error."),'<b>'.esc_html($log_path).'</b>');
    } else {
                    ...
                    <table class="wp-list-table fixed striped" id="error-log">
                        ...
                        <tbody id="the-list"<?php echo (count($error_log) === 0) ? ' style="overflow: hidden;"' : ''; ?>>
                            ...
                            <?php else : ?>
                                ...
                                <tr>
                                    <td scope="col" style="width: 15%; text-align: center"><b class="info" title="<?php echo date("Y-m-d H:i:s T (P)", esc_html(strtotime($log['dateTime']))); ?>"><?php echo esc_html($log['dateTime']); ?></b></td>
