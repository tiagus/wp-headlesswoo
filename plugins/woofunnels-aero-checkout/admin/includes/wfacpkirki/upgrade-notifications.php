...
if ( ! function_exists( 'wfacpkirki_show_upgrade_notification' ) ) :
	...
	function wfacpkirki_show_upgrade_notification( $plugin_data, $response ) {
		...
		if ( isset( $response->upgrade_notice ) && strlen( trim( $response->upgrade_notice ) ) > 0 ) : ?>
			<style>.wfacpkirki-upgrade-notification {background-color:#d54e21;padding:10px;color:#f9f9f9;margin-top:10px;margin-bottom:10px;}.wfacpkirki-upgrade-notification + p {display:none;}</style>
			<div class="wfacpkirki-upgrade-notification">
				<strong><?php esc_attr_e( 'Important Upgrade Notice:', 'wfacpkirki' ); ?></strong>
...
add_action( 'in_plugin_update_message-' . plugin_basename( __FILE__ ), 'wfacpkirki_show_upgrade_notification', 10, 2 );
