var wfacpkirkiDependencies = {
	...
	showKirkiControl: function( control ) {
		var self     = this,
			...
			isOption = (
				...
				control.params.wfacpkirkiOptionType &&  // Check if option_type exists.
				'option' === control.params.wfacpkirkiOptionType &&  // We're using options.
				control.params.wfacpkirkiOptionName && // Check if option_name exists.
				! _.isEmpty( control.params.wfacpkirkiOptionName ) // Check if option_name is not empty.
			),
			i;
	...
	checkCondition: function( requirement, control, isOption, relation ) {
		...
		if ( isOption && requirement.setting ) {
			...
			if ( -1 === requirement.setting.indexOf( control.params.wfacpkirkiOptionName + '[' ) ) {
				requirement.setting = control.params.wfacpkirkiOptionName + '[' + requirement.setting + ']';
			}
		}
