...
class WFACPKirki_Output {
	...
	protected function apply_pattern_replace( $output, $value ) {
		if ( isset( $output['pattern_replace'] ) && is_array( $output['pattern_replace'] ) ) {
			$option_type = ( '' !== WFACPKirki::get_config_param( $this->config_id, 'option_type' ) ) ? WFACPKirki::get_config_param( $this->config_id, 'option_type' ) : 'theme_mod';
			$option_name = WFACPKirki::get_config_param( $this->config_id, 'option_name' );
			$options     = array();
			if ( $option_name ) {
			...
			if ( isset( $output['exclude'] ) && is_array( $output['exclude'] ) ) {
				foreach ( $output['exclude'] as $exclude ) {
					if ( is_array( $value ) ) {
						...
						// Fixes https://github.com/aristath/wfacpkirki/issues/1416.
	...
	protected function process_property_value( $property, $value ) {
		$properties = apply_filters(
			"wfacpkirki_{$this->config_id}_output_property_classnames", array(
