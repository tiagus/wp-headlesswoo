...
final class WFACPKirki_Modules_CSS_Generator {
	...
	private function __construct() {
		if ( is_null( self::$google_fonts ) ) {
			self::$google_fonts = WFACPKirki_Fonts::get_google_fonts();
		}
		if ( is_null( self::$backup_fonts ) ) {
			self::$backup_fonts = WFACPKirki_Fonts::get_backup_fonts();
		}
	}
	...
	public static function css( $field ) {
		...
		self::$value = WFACPKirki_Values::get_sanitized_field_value( $field );

		// Find the class that will handle the outpout for this field.
		$classname            = 'WFACPKirki_Output';
		$field_output_classes = apply_filters( "wfacpkirki_{$field['wfacpkirki_config']}_output_control_classnames", array(
			'wfacpkirki-background' => 'WFACPKirki_Output_Field_Background',
			'wfacpkirki-dimensions' => 'WFACPKirki_Output_Field_Dimensions',
			'wfacpkirki-image'      => 'WFACPKirki_Output_Field_Image',
			'wfacpkirki-typography' => 'WFACPKirki_Output_Field_Typography',
			'wfacpkirki-multicolor' => 'WFACPKirki_Output_Field_Multicolor',
		) );
		if ( array_key_exists( self::$field_type, $field_output_classes ) ) {
			$classname = $field_output_classes[ self::$field_type ];
		}
		$obj = new $classname( $field['wfacpkirki_config'], self::$output, self::$value, $field );
	...
	public static function styles_parse( $css = array() ) {
		...
		// Pass our styles from the wfacpkirki_styles_array filter.
		$css = apply_filters( 'wfacpkirki_styles_array', $css );
