...
class WFACPKirki_CSS_To_File {
	...
	protected function get_path( $context = '' ) {
		...
		$paths = array(
			'file'   => wp_normalize_path( $upload_dir['basedir'] . '/wfacpkirki-css/styles.css' ),
			'folder' => wp_normalize_path( $upload_dir['basedir'] . '/wfacpkirki-css' ),
		);
	...
	public function write_file() {
		...
		foreach ( $configs as $config_id => $args ) {
			...
			$css[ $config_id ] = apply_filters( "wfacpkirki_{$config_id}_dynamic_css", WFACPKirki_Modules_CSS::loop_controls( $config_id ) );
		}
		$css = implode( $css, '' );

		/*
		WIP: Minimize the CSS a bit. Commented-out because of https://github.com/aristath/wfacpkirki/issues/1787
		...
		*/
		...
		if ( ! $write_file ) {
			$this->fallback = true;
			set_transient( 'wfacpkirki_css_write_to_file_failed', true, HOUR_IN_SECONDS );
		}
		return $write_file;
