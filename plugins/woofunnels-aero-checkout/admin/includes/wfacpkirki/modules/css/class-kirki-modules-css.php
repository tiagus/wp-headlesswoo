...
class WFACPKirki_Modules_CSS {
	...
	public function init() {
		...
		WFACPKirki_Modules_Webfonts::get_instance();
		...
		// Allow completely disabling WFACPKirki CSS output.
		if ( ( defined( 'KIRKI_NO_OUTPUT' ) && true === KIRKI_NO_OUTPUT ) || ( isset( $config['disable_output'] ) && true === $config['disable_output'] ) ) {
			return;
		}

		$method = apply_filters( 'wfacpkirki_dynamic_css_method', 'inline' );
		if ( $wp_customize ) {
			...
			if ( 'file' === $method ) {
				$this->css_to_file = new WFACPKirki_CSS_To_File();
		...
		if ( 'file' === $method ) {
			...
			$failed = get_transient( 'wfacpkirki_css_write_to_file_failed' );
		...
		if ( ( true === self::$ajax ) || ( isset( $config['inline_css'] ) && false === $config['inline_css'] ) ) {
			...
			add_action( 'wp_ajax_wfacpkirki_dynamic_css', array( $this, 'ajax_dynamic_css' ) );
			add_action( 'wp_ajax_nopriv_wfacpkirki_dynamic_css', array( $this, 'ajax_dynamic_css' ) );
	...
	public function inline_dynamic_css() {
		...
		if ( ! $this->processed ) {
			foreach ( $configs as $config_id => $args ) {
				...
				$styles = apply_filters( "wfacpkirki_{$config_id}_dynamic_css", $styles );
				if ( ! empty( $styles ) ) {
					$stylesheet = apply_filters( "wfacpkirki_{$config_id}_stylesheet", false );
					if ( $stylesheet ) {
						wp_add_inline_style( $stylesheet, $styles );
						continue;
					}
					wp_enqueue_style( 'wfacpkirki-styles-' . $config_id, trailingslashit( WFACPKirki::$url ) . 'assets/css/wfacpkirki-styles.css', array(), WFACP_KIRKI_VERSION );
					wp_add_inline_style( 'wfacpkirki-styles-' . $config_id, $styles );
				}
			}
			$this->processed = true;
		}

		if ( self::$enqueue_fa && apply_filters( 'wfacpkirki_load_fontawesome', true ) ) {
			wp_enqueue_script( 'wfacpkirki-fontawesome-font', 'https://use.fontawesome.com/30858dc40a.js', array(), '4.0.7', true );
		}
	}
	...
	public function ajax_dynamic_css() {
		require wp_normalize_path( WFACPKirki::$path . '/modules/css/dynamic-css.php' );
		exit;
	}
	...
	public function frontend_styles() {
		wp_enqueue_style( 'wfacpkirki-styles-php', admin_url( 'admin-ajax.php' ) . '?action=wfacpkirki_dynamic_css', array(), WFACP_KIRKI_VERSION );
	}
	...
	public static function loop_controls( $config_id ) {
		...
		// Get an instance of the WFACPKirki_Modules_CSS_Generator class.
		// This will make sure google fonts and backup fonts are loaded.
		WFACPKirki_Modules_CSS_Generator::get_instance();
		...
		foreach ( $fields as $field ) {
			...
			if ( $config_id !== $field['wfacpkirki_config'] ) {
				continue;
			}

			if ( true === apply_filters( "wfacpkirki_{$config_id}_css_skip_hidden", true ) ) {
			...
			if ( isset( $field['output'] ) && ! empty( $field['output'] ) ) {
				$css = WFACPKirki_Helper::array_replace_recursive( $css, WFACPKirki_Modules_CSS_Generator::css( $field ) );

				// Add the globals.
				if ( isset( self::$css_array[ $config_id ] ) && ! empty( self::$css_array[ $config_id ] ) ) {
					WFACPKirki_Helper::array_replace_recursive( $css, self::$css_array[ $config_id ] );
				}
			}
		}

		$css = apply_filters( "wfacpkirki_{$config_id}_styles", $css );

		if ( is_array( $css ) ) {
			return WFACPKirki_Modules_CSS_Generator::styles_parse( WFACPKirki_Modules_CSS_Generator::add_prefixes( $css ) );
		}
	}
