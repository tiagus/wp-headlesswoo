<?php
/**
 * Automatic postMessage scripts calculation for WFACPKirki controls.
 *
 * @package     WFACPKirki
 * @category    Modules
...
class WFACPKirki_Modules_PostMessage {
	...
	public function postmessage() {
		...
		wp_enqueue_script( 'wfacpkirki_auto_postmessage', trailingslashit( WFACPKirki::$url ) . 'modules/postmessage/postmessage.js', array( 'jquery', 'customize-preview' ), WFACP_KIRKI_VERSION, true );
		...
		$this->script = apply_filters( 'wfacpkirki_postmessage_script', $this->script );
		wp_add_inline_script( 'wfacpkirki_auto_postmessage', $this->script, 'after' );
	...
	protected function script( $args ) {
		...
		if ( $add_css ) {
			...
			// The style ID.
			$style_id = 'wfacpkirki-postmessage-' . str_replace( array( '[', ']' ), '', $args['settings'] );
	...
	protected function script_var_typography( $args, $field ) {
		...
		foreach ( $css_build_array as $property => $var ) {
			...
			// Fixes https://github.com/aristath/wfacpkirki/issues/1436.
