...
class WFACPKirki_Control_Repeater extends WFACPKirki_Control_Base {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->row_label = array(
			'type'  => 'text',
			'value' => esc_attr__( 'row', 'wfacpkirki' ),
			'field' => false,
		);
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'wfacpkirki' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_attr__( 'Select a Page', 'wfacpkirki' ),
								'option_none_value' => '0',
								'selected'          => '',
							)
						);
	...
	public function to_json() {
		...
		$this->json['value'] = apply_filters( "wfacpkirki_controls_repeater_value_{$this->id}", $this->json['value'] );
	}
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'wfacpkirki' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
					<# _.each( data, function( field, i ) { #>
						...
						<div class="repeater-field repeater-field-{{{ field.type }}} repeater-field-{{ field.id }}">
							...
							<# } else if ( 'color' === field.type ) { #>
								...
								<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'wfacpkirki' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="wfacpkirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'wfacpkirki' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_attr_e( 'No Image Selected', 'wfacpkirki' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'wfacpkirki' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'wfacpkirki' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'wfacpkirki' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change Image', 'wfacpkirki' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add Image', 'wfacpkirki' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="wfacpkirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'wfacpkirki' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_attr_e( 'No File Selected', 'wfacpkirki' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'wfacpkirki' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'wfacpkirki' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change File', 'wfacpkirki' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add File', 'wfacpkirki' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'wfacpkirki' ); ?></button>
				</div>
			</li>
		</script>
		<?php
