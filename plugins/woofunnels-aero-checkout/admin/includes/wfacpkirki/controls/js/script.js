...
if ( _.isUndefined( window.wfacpkirkiSetSettingValue ) ) {
	var wfacpkirkiSetSettingValue = { // eslint-disable-line vars-on-top
		...
		set: function( setting, value ) {
			...
			switch ( subControl.type ) {
				...
				case 'wfacpkirki-background':
					if ( ! _.isUndefined( value['background-color'] ) ) {
						$this.setColorPicker( $this.findElement( setting, '.wfacpkirki-color-control' ), value['background-color'] );
					}
				...
				case 'wfacpkirki-color':
					$this.setColorPicker( $this.findElement( setting, '.wfacpkirki-color-control' ), value );
					break;

				case 'wfacpkirki-multicheck':
				...
				case 'wfacpkirki-multicolor':
					_.each( value, function( subVal, index ) {
						$this.setColorPicker( $this.findElement( setting, '.multicolor-index-' + index ), subVal );
					} );
					break;

				case 'wfacpkirki-radio-buttonset':
				case 'wfacpkirki-radio-image':
				case 'wfacpkirki-radio':
				case 'wfacpkirki-dashicons':
				case 'wfacpkirki-color-palette':
				case 'wfacpkirki-palette':
					jQuery( $this.findElement( setting, 'input[value="' + value + '"]' ) ).prop( 'checked', true );
					break;

				case 'wfacpkirki-typography':
...
var wfacpkirki = {
	...
	initialize: function() {
		...
		/*setTimeout( function() {
			wfacpkirki.util.webfonts.standard.initialize();
			wfacpkirki.util.webfonts.google.initialize();
		}, 150 );*/
...
// Initialize the wfacpkirki object.
wfacpkirki.initialize();
var wfacpkirki = wfacpkirki || {};
wfacpkirki = jQuery.extend( wfacpkirki, {
	...
	control: {
		...
		'wfacpkirki-radio': {
			...
			init: function( control ) {
				...
				// Init the control.
				wfacpkirki.input.radio.init( control );
			...
			template: function( control ) {
				var template = wp.template( 'wfacpkirki-input-radio' );
				control.container.html( template( {
					...
					value: wfacpkirki.setting.get( control.id ),
		...
		'wfacpkirki-color': {
			...
			init: function( control ) {
				...
				// Init the control.
				wfacpkirki.input.color.init( control );
			 ...
			 */
			template: function( control ) {
				var template = wp.template( 'wfacpkirki-input-color' );
				control.container.html( template( {
					label: control.params.label,
					description: control.params.description,
					'data-id': control.id,
		...
		'wfacpkirki-generic': {
			...
			init: function( control ) {
				...
				if ( ! _.isUndefined( control.params ) && ! _.isUndefined( control.params.choices ) && ! _.isUndefined( control.params.choices.element ) && 'textarea' === control.params.choices.element ) {
					wfacpkirki.input.textarea.init( control );
					return;
				}
				wfacpkirki.input.genericInput.init( control );
			},
			...
			template: function( control ) {
				...
				if ( ! _.isUndefined( control.params ) && ! _.isUndefined( control.params.choices ) && ! _.isUndefined( control.params.choices.element ) && 'textarea' === control.params.choices.element ) {
					template = wp.template( 'wfacpkirki-input-textarea' );
					control.container.html( template( args ) );
					return;
				}
				template = wp.template( 'wfacpkirki-input-generic' );
				control.container.html( template( args ) );
			}
		},
		...
		'wfacpkirki-number': {
			...
			init: function( control ) {
				...
				// Init the control.
				wfacpkirki.input.number.init( control );
			},
			 ...
			 */
			template: function( control ) {
				var template = wp.template( 'wfacpkirki-input-number' );
		...
		'wfacpkirki-select': {
			...
			init: function( control ) {
				...
				// Init the control.
				wfacpkirki.input.select.init( control );
			},
			 ...
			 */
			template: function( control ) {
				var template = wp.template( 'wfacpkirki-input-select' );
...
wfacpkirki = jQuery.extend( wfacpkirki, {
	...
	input: {
		...
		radio: {
			...
			init: function( control ) {
				...
				input.on( 'change keyup paste click', function() {
					wfacpkirki.setting.set( control.id, jQuery( this ).val() );
				} );
			}
		},
		...
		color: {
			 ...
			 */
			init: function( control ) {
				var picker = jQuery( '.wfacpkirki-color-control[data-id="' + control.id + '"]' ),
					clear;
				...
				setTimeout( function() {
					clear = jQuery( '.wfacpkirki-input-container[data-id="' + control.id + '"] .wp-picker-clear' );
					if ( clear.length ) {
						clear.click( function() {
							wfacpkirki.setting.set( control.id, '' );
						} );
					}
				}, 200 );
				...
				picker.wpColorPicker( {
					change: function() {
						...
						setTimeout( function() {
							wfacpkirki.setting.set( control.id, picker.val() );
						}, 20 );
					}
				} );
			}
		...
		select: {
			...
			init: function( control ) {
				...
				jQuery( element ).selectWoo( selectWooOptions ).on( 'change', function() {
					...
					wfacpkirki.setting.set( control.id, selectValue );
				} );
			}
		},
		...
		number: {
			...
			init: function( control ) {
				...
				up   = jQuery( '.wfacpkirki-input-container[data-id="' + control.id + '"] .plus' );
				down = jQuery( '.wfacpkirki-input-container[data-id="' + control.id + '"] .minus' );
		...
		image: {
			...
			getTemplate: function( data ) {
				...
				if ( data.value.url || '' !== url ) {
					...
					html += '<div class="placeholder">' + wfacpkirkiL10n.noFileSelected + '</div>';
				}
				html += '<div class="actions">';
				html += '<button class="button image-upload-remove-button' + ( '' === url ? ' hidden' : '' ) + '">' + wfacpkirkiL10n.remove + '</button>';
				if ( data.default && '' !== data.default ) {
					...
					html += '>' + wfacpkirkiL10n.default + '</button>';
				}
				html += '<button type="button" class="button image-upload-button">' + wfacpkirkiL10n.selectFile + '</button>';
				html += '</div></div>';

				return '<div class="wfacpkirki-input-container" data-id="' + data.id + '">' + html + '</div>';
			},
		...
		webfonts: {
			...
			google: {
				...
				setFonts: function() {
					...
					jQuery.post( ajaxurl, { 'action': 'wfacpkirki_fonts_google_all_get' }, function( response ) {
			...
			standard: {
				...
				setFonts: function() {
					...
					// jQuery.post( ajaxurl, { 'action': 'wfacpkirki_fonts_standard_all_get' }, function( response ) {
					//
					// 	// Get fonts from the JSON array.
					// 	self.fonts = JSON.parse( response );
					// } );
...
( function() {
	 ...
	 */
	wp.customize.wfacpkirkiDynamicControl = wp.customize.Control.extend( {
		...
		initialize: function( id, options ) {
			...
			if ( ! args.params.type ) {
				args.params.type = 'wfacpkirki-generic';
			}
		...
		embed: function() {
			...
			wp.customize.section( sectionId, function( section ) {
				if ( 'wfacpkirki-expanded' === section.params.type || section.expanded() || wp.customize.settings.autofocus.control === control.id ) {
		...
		initKirkiControl: function( control ) {
			if ( 'undefined' !== typeof wfacpkirki.control[ control.params.type ] ) {
				wfacpkirki.control[ control.params.type ].init( control );
				return;
			}
...
_.each( wfacpkirki.control, function( obj, type ) {
	wp.customize.controlConstructor[ type ] = wp.customize.wfacpkirkiDynamicControl.extend( {} );
} );
/* global wfacpkirkiControlLoader */
wp.customize.controlConstructor['wfacpkirki-background'] = wp.customize.Control.extend( {
	...
	ready: function() {
		...
		// Init the control.
		if ( ! _.isUndefined( window.wfacpkirkiControlLoader ) && _.isFunction( wfacpkirkiControlLoader ) ) {
			wfacpkirkiControlLoader( control );
		} else {
	...
	initKirkiControl: function() {
		...
		var control = this,
			value   = control.setting._value,
			picker  = control.container.find( '.wfacpkirki-color-control' );
...
wp.customize.controlConstructor['wfacpkirki-color-palette'] = wp.customize.wfacpkirkiDynamicControl.extend( {} );
wp.customize.controlConstructor['wfacpkirki-dashicons'] = wp.customize.wfacpkirkiDynamicControl.extend( {} );
wp.customize.controlConstructor['wfacpkirki-date'] = wp.customize.wfacpkirkiDynamicControl.extend( {
	...
	initKirkiControl: function() {
		...
		control.container.find( '.wfacpkirki-controls-loading-spinner' ).hide();
...
/* global dimensionwfacpkirkiL10n */
wp.customize.controlConstructor['wfacpkirki-dimension'] = wp.customize.wfacpkirkiDynamicControl.extend( {
	...
	initKirkiControl: function() {
		...
		// Notifications.
		control.wfacpkirkiNotifications();
	 ...
	 */
	wfacpkirkiNotifications: function() {
		...
		wp.customize( control.id, function( setting ) {
			setting.bind( function( value ) {
				...
				if ( false === wfacpkirki.util.validate.cssValue( value ) ) {
					setting.notifications.add( code, new wp.customize.Notification(
						code,
						{
							type: 'warning',
							message: dimensionwfacpkirkiL10n['invalid-value']
						}
					) );
				} else {
...
/* global dimensionswfacpkirkiL10n */
wp.customize.controlConstructor['wfacpkirki-dimensions'] = wp.customize.wfacpkirkiDynamicControl.extend( {
		...
		wp.customize( control.id, function( setting ) {
			setting.bind( function( value ) {
				...
				_.each( value, function( val, direction ) {
					if ( false === wfacpkirki.util.validate.cssValue( val ) ) {
						subs[ direction ] = val;
					} else {
				...
				if ( ! _.isEmpty( subs ) ) {
					message = dimensionswfacpkirkiL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
...
/* global tinyMCE */
wp.customize.controlConstructor['wfacpkirki-editor'] = wp.customize.wfacpkirkiDynamicControl.extend( {

	initKirkiControl: function() {

		var control = this,
			element = control.container.find( 'textarea' ),
			id      = 'wfacpkirki-editor-' + control.id.replace( '[', '' ).replace( ']', '' ),
			editor;
...
wp.customize.controlConstructor['wfacpkirki-fontawesome'] = wp.customize.wfacpkirkiDynamicControl.extend( {
...
/* global wfacpkirkiControlLoader */
wp.customize.controlConstructor['wfacpkirki-image'] = wp.customize.Control.extend( {
...
wp.customize.controlConstructor['wfacpkirki-multicheck'] = wp.customize.wfacpkirkiDynamicControl.extend( {
...
/* global wfacpkirkiControlLoader */
wp.customize.controlConstructor['wfacpkirki-multicolor'] = wp.customize.Control.extend( {
	...
	initKirkiControl: function() {
		...
		function wfacpkirkiMulticolorChangeHandler( control, value, subSetting ) {
		...
		while ( i < Object.keys( colors ).length ) {
			wfacpkirkiMulticolorChangeHandler( this, value, keys[ i ] );
			i++;
		}
	},
...
wp.customize.controlConstructor['wfacpkirki-palette'] = wp.customize.wfacpkirkiDynamicControl.extend( {} );
wp.customize.controlConstructor['wfacpkirki-radio-buttonset'] = wp.customize.wfacpkirkiDynamicControl.extend( {} );
wp.customize.controlConstructor['wfacpkirki-radio-image'] = wp.customize.wfacpkirkiDynamicControl.extend( {} );
/* global wfacpkirkiControlLoader */
...
wp.customize.controlConstructor.repeater = wp.customize.Control.extend( {
	...
	initKirkiControl: function() {
		...
		// The current value set in Control Class (set in WFACPKirki_Customize_Repeater_Control::to_json() function)
	...
	setImageInRepeaterField: function( attachment ) {
		...
		$targetDiv.find( '.wfacpkirki-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ) {
		...
		$targetDiv.find( '.wfacpkirki-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ) {
		...
		$targetDiv.find( '.wfacpkirki-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ) {
		...
		$targetDiv.find( '.wfacpkirki-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		} );
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
	...
	addRow: function( data ) {
		...
		var control       = this,
			template      = control.repeaterTemplate(), // The template for the new row (defined on WFACPKirki_Customize_Repeater_Control::render_content() ).
...
wp.customize.controlConstructor['wfacpkirki-slider'] = wp.customize.wfacpkirkiDynamicControl.extend( {
...
/* global wfacpkirkiControlLoader */
wp.customize.controlConstructor['wfacpkirki-sortable'] = wp.customize.Control.extend( {
...
wp.customize.controlConstructor['wfacpkirki-switch'] = wp.customize.wfacpkirkiDynamicControl.extend( {
...
wp.customize.controlConstructor['wfacpkirki-toggle'] = wp.customize.wfacpkirkiDynamicControl.extend( {
...
/* global wfacpkirkiL10n, wfacpkirki */
wp.customize.controlConstructor['wfacpkirki-typography'] = wp.customize.wfacpkirkiDynamicControl.extend( {
	...
	initKirkiControl: function() {
		...
		if ( ! _.isUndefined( control.params.default.color ) ) {
			picker = this.container.find( '.wfacpkirki-color-control' );
			picker.wpColorPicker( {
				change: function() {
					setTimeout( function() {
	...
	renderFontSelector: function() {
		...
		data = jQuery.extend( {}, controlFontFamilies, {
			default: {
				text: wfacpkirkiL10n.defaultCSSValues,
				children: [
					{ id: '', text: wfacpkirkiL10n.defaultBrowserFamily },
					{ id: 'initial', text: 'initial' },
					{ id: 'inherit', text: 'inherit' }
				]
			},
			standard: {
				text: wfacpkirkiL10n.standardFonts,
				children: standardFonts
			},
			google: {
				text: wfacpkirkiL10n.googleFonts,
				children: googleFonts
			}
		} );

		if ( wfacpkirkiL10n.isScriptDebug ) {
			console.info( 'WFACPKirki Debug: Font families for control "' + control.id + '":' );
			console.info( data );
		}
	...
	renderBackupFontSelector: function() {
		...
		if ( 'inherit' === fontFamily || 'initial' === fontFamily || 'google' !== wfacpkirki.util.webfonts.getFontType( fontFamily ) ) {
	...
	renderVariantSelector: function() {
		...
		var control    = this,
			...
			data       = [],
			isValid    = false,
			fontType   = wfacpkirki.util.webfonts.getFontType( fontFamily ),
		...
		if ( 'google' === fontType ) {
			variants = wfacpkirki.util.webfonts.google.getVariants( fontFamily );
		}
		...
		if ( wfacpkirkiL10n.isScriptDebug ) {
			console.info( 'WFACPKirki Debug: Font variants for font-family "' + fontFamily + '":' );
	...
	getFonts: function() {
		var control            = this,
			initialGoogleFonts = wfacpkirki.util.webfonts.google.getFonts(),
			googleFonts        = {},
		...
		if ( ! _.isEmpty( control.params.choices.fonts.google ) ) {
			if ( 'alpha' === control.params.choices.fonts.google[0] || 'popularity' === control.params.choices.fonts.google[0] || 'trending' === control.params.choices.fonts.google[0] ) {
				...
				googleFonts = wfacpkirki.util.webfonts.google.getFonts( googleFontsSort, '', googleFontsNumber );
		...
		if ( ! _.isEmpty( control.params.choices.fonts.standard ) ) {
			_.each( control.params.choices.fonts.standard, function( fontName ) {
				if ( 'undefined' !== typeof wfacpkirki.util.webfonts.standard.fonts[ fontName ] && ! _.isEmpty( wfacpkirki.util.webfonts.standard.fonts[ fontName ] ) ) {
					standardFonts[ fontName ] = {};
					if ( 'undefined' !== wfacpkirki.util.webfonts.standard.fonts[ fontName ].stack && ! _.isEmpty( wfacpkirki.util.webfonts.standard.fonts[ fontName ].stack ) ) {
						standardFonts[ fontName ].family = wfacpkirki.util.webfonts.standard.fonts[ fontName ].stack;
					} else {
						standardFonts[ fontName ].family = googleFonts[ fontName ];
					}
					if ( 'undefined' !== wfacpkirki.util.webfonts.standard.fonts[ fontName ].label && ! _.isEmpty( wfacpkirki.util.webfonts.standard.fonts[ fontName ].label ) ) {
						standardFonts[ fontName ].label = wfacpkirki.util.webfonts.standard.fonts[ fontName ].label;
			...
			_.each( wfacpkirki.util.webfonts.standard.fonts, function( font, id ) {
				standardFonts[ id ] = {
					family: font.stack,
					label: font.label
				};
	...
	localFontsCheckbox: function() {
		var control           = this,
			checkboxContainer = control.container.find( '.wfacpkirki-host-font-locally' ),
			checkbox          = control.container.find( '.wfacpkirki-host-font-locally input' ),
