...
class WooFunnels_License_check {
	...
	public function update_plugins( $data ) {
		return WooFunnels_License_Controller::update_plugins( $data );
	}
	...
	public function handle_license_check_response( $output ) {
		if ( false !== $output ) {
			...
			if ( $output['status_check'] && '3' == $output['status_check'] ) {
				...
				$this->remove_license();
			} elseif ( '2' == $output['status_check'] ) {
				$this->mark_license_expired();

			} elseif ( '0' == $output['status_check'] ) {
