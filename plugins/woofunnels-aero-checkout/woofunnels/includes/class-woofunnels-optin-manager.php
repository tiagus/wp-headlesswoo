...
class WooFunnels_OptIn_Manager {
	...
	public static function init() {
		...
		// track usage user callback
		add_action( 'bwf_maybe_track_usage_scheduled_current', array( __CLASS__, 'maybe_track_usage' ) );
	...
	public static function collect_data() {
		...
		return apply_filters( 'woofunnels_global_tracking_data', array(
			'url'                    => home_url(),
	...
	public static function maybe_clear_optin() {
		...
		if ( isset( $_GET['woofunnels_tracking'] ) && ( 'reset' == $_GET['woofunnels_tracking'] ) ) {
			self::reset_optin();
			wp_redirect( admin_url( 'admin.php?page=woofunnels&tab=tools' ) );
			exit;
		}
	}
	...
	public static function initiate_schedules() {
		...
		if ( 'yes' === self::get_optIn_state() && ! wp_next_scheduled( 'bwf_maybe_track_usage_scheduled' ) ) {
