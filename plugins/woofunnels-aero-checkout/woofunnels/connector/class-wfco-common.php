...
class WFCO_Common {
	...
	public static function get_connectors_data() {
		$temp_arr = array();
		/** @todo need to add transient as fetching calls continuously */
		$response = WFCO_Model_Connectors::get_results( 'SELECT * FROM {table_name}' );
		if ( is_array( $response ) && count( $response ) > 0 ) {
			foreach ( $response as $connector ) {
				$temp_arr[ $connector['slug'] ]['id']        = $connector['ID'];
				$temp_arr[ $connector['slug'] ]['last_sync'] = $connector['last_sync'];
				$temp_arr[ $connector['slug'] ]['status']    = $connector['status'];

				$settings_meta = WFCO_Model_ConnectorMeta::get_rows( true, array( $connector['ID'] ) );
				foreach ( $settings_meta as $meta ) {
					$temp_arr[ $connector['slug'] ][ $meta['meta_key'] ] = self::get_metakey_value( $settings_meta, $meta['meta_key'] );
				}
			}
		}
