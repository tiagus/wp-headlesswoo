...
class WFCO_AJAX_Controller {
	...
	public static function handle_admin_ajax() {
		add_action( 'wp_ajax_wfco_save_connector', array( __CLASS__, 'save_connector' ) );
		add_action( 'wp_ajax_wfco_sync_connector', array( __CLASS__, 'sync_connector' ) );
		add_action( 'wp_ajax_wfco_delete_connector', array( __CLASS__, 'delete_connector' ) );
		add_action( 'wp_ajax_wfco_update_connector', array( __CLASS__, 'update_connector' ) );
		add_action( 'wp_ajax_wfco_connector_install', array( __CLASS__, 'connector_install' ) );
		add_action( 'wp_ajax_wfco_create_connector_license', array( __CLASS__, 'create_connector_license' ) );
	}
	...
	public static function save_connector() {
		...
		if ( isset( $_REQUEST['wfco_connector'] ) && '' != $_REQUEST['wfco_connector'] && wp_verify_nonce( $_REQUEST['_wpnonce'], 'wfco-connector' ) ) {
			$active_connectors = WFCO_Load_Connectors::get_active_connectors();
			$response          = $active_connectors[ $_REQUEST['wfco_connector'] ]->handle_settings_form( $_REQUEST, 'save' );
	...
	public static function sync_connector() {
		...
		if ( isset( $_REQUEST['slug'] ) && '' != $_REQUEST['slug'] && isset( $_REQUEST['id'] ) && '' != $_REQUEST['id'] && wp_verify_nonce( $_REQUEST['sync_nonce'], 'wfco-connector-sync' ) ) {

			$slug              = trim( $_REQUEST['slug'] );
			$active_connectors = WFCO_Load_Connectors::get_connector( $slug );

			$data = [];
			try {
				$data = $active_connectors->handle_settings_form( $_REQUEST, 'sync' );
			...
			if ( count( $data ) > 0 ) {
				$resp['status']       = true;
				$resp['id']           = $data['id'];
				$resp['data_changed'] = $data['data_changed'];
				$resp['redirect_url'] = add_query_arg( array(
					'page' => 'connector',
				), admin_url( 'admin.php' ) );
				wp_send_json( $resp );
	...
	public static function delete_connector() {
		...
		if ( isset( $_REQUEST['id'] ) && '' != $_REQUEST['id'] && wp_verify_nonce( $_REQUEST['delete_nonce'], 'wfco-connector-delete' ) ) {

			$connector_id      = $_REQUEST['id'];
			$connector_details = WFCO_Model_Connectors::get( $_REQUEST['id'] );
			$connector_slug    = $connector_details['slug'];
			$sql_query         = 'DELETE from {table_name} where connector_id = %d';
			$sql_query         = $wpdb->prepare( $sql_query, $connector_id );
	...
	public static function connector_install() {
		...
		if ( isset( $_REQUEST['connector_slug'] ) && '' != $_REQUEST['connector_slug'] && wp_verify_nonce( $_REQUEST['install_nonce'], 'wfco-connector-install' ) ) {

			$connector_slug    = trim( $_REQUEST['connector_slug'] );
			$connector_slug    = trim( $_REQUEST['slug'] );
			$resp['connector'] = $connector_slug;
			...
			if ( empty( $licence_data ) ) {
				$resp['msg']        = __( 'No License found', 'woofunnels-autobot-automation' );
				$resp['error_code'] = 101;
				wp_send_json( $resp );
			}

			$licence       = $licence_data['data_extra']['api_key'];
			$connector_api = new WFCO_Connector_api( $licence, $connector_slug );
			...
			$connector_plugin_file   = $all_connector[ $connector_slug ]['file'];
			...
			if ( $result ) {
				...
				$resp['status'] = false;
				$resp['msg']    = __( 'Some error occurred during downloading & installing the connector, kindly contact woofunnels team.', 'woofunnels-autobot-automation' );
			}
	...
	public static function create_connector_license() {
		...
		$resp['status'] = false;
		if ( isset( $_REQUEST['connector_slug'] ) && '' != $_REQUEST['connector_slug'] ) {
			$connector_slug  = trim( $_REQUEST['connector_slug'] );
			$connector_slugs = trim( $_REQUEST['slug'] );
			...
			$licence       = $licence_data['data_extra']['api_key'];
			$connector_api = new WFCO_Connector_api( $licence, $connector_slug, 'create_connector_license' );
			$connector_api->create_license();
			$response_data = $connector_api->get_package();
			if ( ! is_array( $response_data ) || ! wc_string_to_bool( $response_data['status'] ) ) {
				$resp['msg'] = __( 'There was some error. Please try again later.1', 'woofunnels-autobot-automation' );
				wp_send_json( $resp );
			}
			...
			if ( $defined_vars['user'][ $plugin_encode_constant ] ) {
				...
				$resp['status'] = true;
				$resp['msg']    = __( 'License Activated', 'woofunnels-autobot-automation' );
			}
			wp_send_json( $resp );
		}
	}
